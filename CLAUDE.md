...
### 분업안 — 작성 위치와 형식 (2026-09-08 신설)

분업은 **말이나 채팅으로 정하고 끝내지 않는다. 정한 즉시 `docs/worksplit.md`에 쓰고 커밋·push한다.**
실제로 2026-09-08에 채팅으로 전체 분업(7항목/9항목)을 확정해놓고 어느 파일에도 안 남겨서 통째로 날린 적이 있다
— 상대 세션은 채팅을 못 보고 문서만 읽으므로, 문서에 없는 분업은 존재하지 않는 것과 같다.

- **위치**: `docs/worksplit.md` 하나만 쓴다(스프린트마다 새 파일을 만들지 않는다 — 옛 파일이 남아 어느 게 최신인지 모르게 된다). 완료된 항목은 지우지 말고 `✅`로 표시해 이력을 남긴다.
- **연결**: `PROGRESS.md`의 "다음 할 일"에서 이 파일을 링크한다. 새 세션은 `CLAUDE.md`→`PROGRESS.md`→`HANDOFF.md` 순으로 읽으므로, 링크가 없으면 안 읽힌다.
- **독자는 "맥락이 0인 다른 클로드 세션"이다.** 채팅에서 오간 합의·이유를 문서 안에 다시 적는다. "저번에 얘기한 대로"는 금지.

**반드시 들어가야 하는 9개 항목** (하나라도 빠지면 상대 세션이 되묻거나 잘못 만든다):

1. **머리말** — 무엇에 대한 분업인지, 근거 문서 링크, 확정 날짜
2. **착수 전제** — 누가 무엇을 먼저 `main`에 올려야 나머지가 시작 가능한지(선행 커밋)
3. **공유 계약** — 양쪽이 같이 쓰는 타입·메서드 시그니처·정적 클래스·태그를 **코드 블록으로 고정**. "임의로 이름/타입 바꾸지 말 것" 명시
4. **트랙별 담당** — 담당자, 목표 한 줄, 번호 매긴 작업 항목(각 항목에 원본 줄 인용 `project_test.html:NNNN`)
5. **건드리는 파일 / 건드리면 안 되는 파일** — 트랙마다 명시. 이게 병합 충돌 방지의 핵심이다
6. **완료 정의(DoD)** — 무엇이 되면 끝인지 + 어떤 테스트로 증명하는지
7. **확정된 세부 결정** — 이미 논의가 끝난 사항. 상대가 다시 묻거나 뒤집지 않게 결론만 적는다
8. **병합 순서 + 씬/프리팹 소유권** — 누가 먼저 `main`에 합치는지, `.unity`/기존 `.prefab`은 누가 만지는지(협업 규칙 1·2번)
9. **범위 밖** — 이번에 하지 않을 것. 안 적으면 상대가 임의로 확장한다

## 코딩/파일 정리 규칙

최종 목표(캐릭터 4종·가챠·아이템 35종·지역 9종·시간 회귀까지 포팅)를 감안하면 파일 수가 많이 늘어난다. 지금(스크립트 10개 이하)은 아직 안 아프지만, 여기서 정한 기준은 **"지금 하기 귀찮아도 나중에 편한" 쪽으로 고른 것**이다 — 나중에 "번거롭다"며 되돌리지 말 것.
...
원본의 `meta` 오브젝트(`project_test.html:1129` `function defaultMeta()`)를 실제로 읽어보면 레벨·골드·**아이템 보유 개수**(`meta.items`, `:1148` "// { 아이템id: 보유 개수 }")·회귀 횟수·지역별 진행·업적·설정까지 전부 담겨 있고, `localStorage.setItem(SAVE_KEY, JSON.stringify(meta))`(`:1214`)로 저장된다 — **플레이어마다 다르고 런타임에 계속 바뀌는 진행 상태**다. 이건 위 "데이터(밸런스 값)" 규칙이 말하는 것과 다른 카테고리라 섞으면 안 된다:
...
- **세이브 데이터**(가변, 플레이어마다 다름, 파일로 직렬화) → SO가 아니다. `PlayerProfile` 같은 **순수 직렬화 가능 C# 클래스**(`[System.Serializable]`, `MonoBehaviour`도 `ScriptableObject`도 아님)로 관리하고 `Systems/`의 세이브 시스템이 파일 입출력을 담당한다. 예: 레벨·골드, **아이템 보유 개수**(정의가 아니라 "몇 개 갖고 있나"), 회귀 횟수, 지역별 진행도, 달성한 업적, 설정값.
...
- `PlayerProfile`은 `Core/`에 둔다(모든 도메인이 자기 슬라이스를 읽어야 함 — `Characters`는 레벨/골드, `Systems`는 아이템/시간 회귀/지역, `UI`는 표시용으로 전부). 실제 파일 입출력(직렬화·역직렬화)은 `Systems/`의 세이브 시스템이 담당 — `Core`는 데이터 형태만 정의하고 로직은 `Systems`(현재 계층표 그대로, 구조 변경 필요 없음).
- **여기서 지금 못박는 것도 카테고리 분류뿐이다**("SO 아님, `Core`에 위치하는 순수 직렬화 클래스"). `PlayerProfile`의 정확한 필드 목록(레벨·골드·아이템 보유량·회귀 횟수·지역 진행·업적 중 정확히 무엇이 언제 추가되는지)은 **Health/아이템/가챠/시간 회귀 각 스프린트가 실제로 시작될 때 그 스프린트의 HANDOFF.md에서 그때그때 확정한다.** 원본 `meta`(`:1129`)를 지금 통째로 옮겨적지 않는다 — 그러면 아직 스코프에 없는 시스템(가챠·시간 회귀 등)까지 미리 설계하게 되어 "HANDOFF.md에 없는 시스템은 손대지 않는다" 규칙과 어긋난다.
...
- **진행 단계(2026-09-10)**: **게임 한 바퀴가 전부 돈다** — 로비 7탭 → 지역 선택 → 사냥 → 결과 → 로비,
  그리고 사냥 → 지역 토벌 → 보스 → 다음 지역 → 시간 회귀 → 뽑기까지 성장 고리가 닫혔다. 미병합 브랜치 없음.
