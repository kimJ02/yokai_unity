...
`Assets/Scenes/CombatCore.unity`를 열면 플레이어(파란 원)가 원본과 동일한 조작(←/→ 이동, **C만** 점프, Z 길게 눌러 차지 후 발사)으로 실제 Physics2D 중력을 받으며 원본 발판 15개(원웨이, 층간 1.35유닛)를 오르내리고, 마법탄으로 Enemy 태그 대상을 관통 공격 가능. 몬스터도 3.6초마다 웨이브 스폰돼 GroundY에서 X축으로만 플레이어를 쫓아오고, 빨간 원으로 표시됨. PlayMode 테스트 **10/10 통과**.

Part B 병합 직후 컴파일 에러 포함 5건이 남아있었는데(아래 로그 참고) 전부 수정 완료. 남은 건 push뿐 — 사용자 최종 확인 후 진행.
...
1. **사용자 확인 후 `git push origin main`** — 지금 로컬 커밋 상태로 배치 컴파일 + PlayMode 10/10 통과까지 확인됐지만, Unity 에디터로 직접 플레이해서 확인한 적은 없음(에디터가 잠겨 있어서 전부 배치모드로만 검증함). 실제로 열어서 한 번 플레이해보길 권장.
...
- [x] 위 항목 다 붙어서 핵심 루프 한 번 플레이 가능 — 배치모드로 검증(PlayMode 10/10). **에디터로 직접 플레이 확인은 아직 안 함**(위 "다음 할 일" 참고)
...
- **이번 병합·수정은 전부 배치모드로만 검증했다** — Unity 에디터 GUI로 직접 Play해서 눈으로 확인한 적은 없다(에디터가 잠겨 있어서). 논리적으로는 PlayMode 테스트 10/10이 통과했지만, 실제 화면에서 웨이브 스폰이 눈에 잘 보이는지·발판 원웨이가 실감상 자연스러운지 등은 사람이 한 번 봐야 확실하다.
