...
**전투 코어 + 체력/피해 + 성장곡선 + 마법사 스킬트리 + 플레이어 뼈대 + 메카닉 0차 + 몬스터 7종 전부가
`main`에 병합·검증 완료.** `main`이 유일한 최신 브랜치이고, 미병합 작업은 없다. **PlayMode 94/94 통과.**

지금 게임은 이렇게 동작한다: `Assets/Scenes/CombatCore.unity`를 열면 플레이어(파란 원)가 ←/→ 이동,
**C**로 점프(원본은 C+Space지만 **의도적 편차로 C만**), **Z** 공격. 원본 발판 15개
(원웨이, 층간 1.35유닛 — 의도적 편차)를 실제 Physics2D 중력으로 오르내린다.
**Tab**으로 마법사↔메카닉 전환(임시 디버그 키 — 로비 구현 때 삭제).
**6/7** 키로 마법사 빌드(폭발/중력)를 배우면 Z 공격이 바뀌고 **X** 스킬이 열린다(이것도 삭제 예정).
...
5. **다음: 섬영 0차 → 드루이드 0차** — 단계 1에서 유일하게 남은 조각.
...
6. **그 다음: 로비** — 캐릭터 선택·강화·윤회·뽑기. 스펙은 `HANDOFF.md` 7번.
   **2026-09-10 재지시로 5번 뒤로 밀렸다** — 캐릭터 선택 화면을 2종으로 만들었다가
   4종으로 다시 만드는 걸 피하기 위해서. 로비가 생기면 임시 디버그 키(Tab 캐릭터 전환,
   6/7 빌드 선택, 숫자키 강화)를 정식 UI로 대체하며 **삭제**한다.
