...
**전투 코어 + 체력/피해 + 성장곡선 + 마법사 스킬트리 + 플레이어 뼈대 + 메카닉 0차 + 몬스터 7종
+ 런 사이클 + 로비 UI가 전부 `main`에 병합·검증 완료.** 미병합 작업 없음. **PlayMode 113/113 통과.**
...
5-1. ✅ **[나] 전투 배수 체인 + 세이브/로드** (`6d96e51`·`ffabf52`, PlayMode 130/130).
   `Core/CombatModifiers`(콤보·살기·연쇄·레벨페널티·성소버프), 히트스톱, `Systems/SaveService`.
6. **다음: 섬영 0차 → 드루이드 0차** — 단계 1에서 유일하게 남은 조각.
