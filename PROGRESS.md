...
1. **사용자가 에디터에서 직접 플레이해서 스프린트 2(체력·데미지) 확인** — 배치 컴파일 + PlayMode 30/30은 통과했지만 에디터 직접 플레이는 아직. 확인 포인트: 적이 한 방에 안 죽고 여러 번 때려야 죽는지, 맞을 때 흰색으로 번쩍이며 뒤로 밀리는지, 적과 부딪히면 플레이어가 밀려나며 체력이 깎이는지(체력바는 아직 없어 눈으로 안 보임 — 스프린트 3에서 HUD 추가), 0.9초 무적 덕에 겹쳐 있어도 즉사하지 않는지.
   - 이전 항목(이동/점프 디테일 + 리팩터링)도 계속 확인 중 — 물리 밀어내기 이슈는 팀원이 수정 완료(2026-08-29 로그).
2. **다음은 스프린트 3(런 사이클 & 보상)** — 제한시간 90초·종료 사유 3종·결과 화면, 골드/경험치/레벨, 최소 HUD(`UI` 도메인 신설). 스프린트 2가 끝났으니 착수 전 `HANDOFF.md`를 스프린트 3 스펙으로 교체하고 지금 것은 `docs/sprints/02-health-damage.md`로 옮긴다.
3. **✅ 완료(2026-09-03): 스프린트 2 = 체력 & 데미지.** 원래 확정 경위는 아래 — 사용자가 `docs/ROADMAP.md`의 권장 순서를 승인하고 "1번부터 시작"으로 지시. 스펙은 새 `HANDOFF.md` 참고.
...
- **2026-09-03** — **스프린트 2 완료: 체력 & 데미지 (PlayMode 30/30).** `feature/health-damage` 브랜치에서 작업.
  - 신설: `Core/IDamageable`(피격 인터페이스) · `Combat/DamageCalculator`(치명타 10%·배수 1.7·±10% 난수·최소 1) · `Enemies/EnemyHealth`(체력 38·넉백·피격 플래시) · `Characters/PlayerHealth`(체력 100·무적 0.9초·피격 반동).
  - 전환: `MageProjectile`/`PlayerAttack`이 `Destroy()` 직접 호출 → `IDamageable.TakeDamage()`. `EnemyMove.TryAttack()` 빈 스텁 → 실제 접촉 데미지(원본처럼 쿨다운 없이 매 프레임, 반복 방지는 플레이어 무적시간 전담).
