...
`Assets/Scenes/CombatCore.unity`를 열면 플레이어(파란 원)가 원본과 동일한 조작(←/→ 이동, **C만** 점프, Z 길게 눌러 차지 후 발사)으로 실제 Physics2D 중력을 받으며 원본 발판 15개(원웨이, 층간 1.35유닛)를 오르내리고, 마법탄으로 Enemy 태그 대상을 관통 공격 가능. 몬스터도 3.6초마다 웨이브 스폰돼 원본처럼 발판/바닥그리드 스폰 포인트(균등 랜덤 아님) 중 하나의 **실제 높이**에서 나타나 물리로 그 위에 서고, 스폰 직후 2초 무적, 추적범위(3유닛) 안이면 플레이어를 쫓고 밖이면 배회하며, 발판 위에서는 가장자리에서 되돌아가 걸어서 떨어지지 않음. PlayMode 테스트 **14/14 통과**.
...
1. **사용자가 다시 플레이해서 스폰/이동 로직 수정 확인 → `git push origin main`** — 배치 컴파일 + PlayMode 14/14는 통과했지만 아직 에디터로 직접 플레이 확인 전.
...
- [x] 위 항목 다 붙어서 핵심 루프 한 번 플레이 가능 — 배치모드로 검증(PlayMode 14/14). 스폰/이동 로직은 사용자가 직접 플레이해서 원본과 다른 점을 두 차례 지적 → 재수정함(위 로그 참고), **이 수정에 대한 재확인은 아직**(위 "다음 할 일" 참고)
...
- **스폰/이동 로직 재수정(2026-08-26)은 아직 에디터로 직접 플레이 확인 전** — 배치 컴파일 + PlayMode 14/14만 통과 확인. 이전 버전들은 사용자가 직접 플레이해보고 원본과 다른 점을 두 번 지적해서 발견됐으니(스폰 위치 균등 랜덤 → 정해진 포인트, 그다음 "발판 위 스폰"이 실제로는 X만 빌리고 Y는 항상 바닥이었던 것), 이번 수정도 실제로 플레이해서 확인 필요.
