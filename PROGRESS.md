...
2. **✅ 다음 스프린트 확정(2026-08-29): 스프린트 2 = 체력 & 데미지.** 사용자가 `docs/ROADMAP.md`의 권장 순서를 승인하고 "1번부터 시작"으로 지시. 스펙은 새 `HANDOFF.md` 참고.
   - 전체 순서: **① 체력/데미지 → ② 런 사이클·보상 → ③ 세이브·로비·골드강화 → ④ SO 전환 → ⑤ 적 종류 확대 → ⑥ 이후(지역·보스·윤회·가챠·캐릭터)**
   - 근거: `docs/ROADMAP.md`(원본 7199줄 전수 분석 + 의존성 분석). 거의 모든 시스템이 체력에 의존하고, ①~③이 끝나야 "사냥→보상→강화→재도전" 최소 완결 루프가 성립한다.
   - 문서 구조 정리: 기존 HANDOFF.md(스프린트 1 스펙)를 `docs/sprints/01-combat-core.md`로 옮기고, HANDOFF.md는 현재 스프린트(2번) 스펙으로 교체 — CLAUDE.md "문서 구조" 규칙대로.
...
- **2026-08-29** — **원본 전수 분석 → 포팅 로드맵 작성 + 스프린트 2 확정·착수.** 사용자가 "프로토타입 전체적으로 분석하고 어떻게 업데이트할지 문서로 작성"을 요청.
  ...
  - 사용자가 이 순서를 승인하고 1번부터 착수 지시 → **스프린트 2(체력 & 데미지) 확정.**
  - CLAUDE.md "문서 구조" 규칙대로 기존 HANDOFF.md를 `docs/sprints/01-combat-core.md`로 `git mv`(스프린트 1 스펙의 확정 수치·의도적 편차는 그대로 유효, 새 HANDOFF.md 상단에서 참조), HANDOFF.md는 스프린트 2 스펙으로 교체. 스펙의 모든 수치는 원본에서 직접 확인해 줄 번호와 함께 기재(넉백 240px/s·감쇠 9/s, 무적 0.9초, 피격 반동 260/-220, 오니 hp38·dmg13, 치명타 10%/1.7배, 데미지 ±10% 난수 등).
