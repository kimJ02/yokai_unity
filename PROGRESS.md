...
**1단계 — 전투 코어 프로토타입, Part A + Part B 전부 `main`에 병합·검증·`origin` push까지 완료. 대규모 리팩터링(폴더/네임스페이스/asmdef 분리 + Monster→Enemy 리네임)도 완료.** 사용자가 직접 플레이해보며 확인 중 — 첫 이슈로 플레이어·몬스터가 서로 밀어내는 문제를 발견해 물리 레이어 분리로 수정·확인 완료(아래 "다음 할 일", 로그 2026-08-29 참고). 계속 플레이하며 추가 확인 예정.
...
1. **사용자가 에디터에서 직접 플레이해서 이번 세션 전체 수정(이동/점프 디테일 + 리팩터링) 확인** — (2026-08-29) 실제로 플레이해서 확인 진행 중. 첫 이슈(플레이어·몬스터 물리 밀어내기)는 발견·수정·재확인 완료(아래 로그 참고). 나머지(이동/점프 디테일 등)는 계속 플레이하며 이상 있으면 리포트 예정 — 완전히 끝난 건 아님.
2. **✅ 스프린트 2 방향 확정(2026-09-08): 팀원 초안(`HANDOFF_sprint2_draft.md`, "성장곡선 검증")을 채택.** 초안의 "1. 체력 시스템" 항목은 내가 이미 `feature/health-damage` 브랜치에 구현·검증(PlayMode 30/30) 완료한 것과 사실상 동일 범위라 — **그걸 먼저 `main`에 병합하고 그 위에 나머지(처치보상·레벨·골드강화·난이도스케일링)를 쌓기로 함.**
   - **트랙 분업**: 트랙 A(팀원) = 처치 보상(골드+EXP) + 난이도 스케일링("가상 지역 레벨"), 트랙 B(나) = 레벨업(`expCurve`) + 골드 강화 5종 + 파생 스탯 적용.
   - **선행 작업(0단계, 아직 미착수)**: ① `feature/health-damage` → `main` 병합 ② `Core/PlayerProfile` 필드 스캐폴딩(로직 없이 필드만 — 두 트랙의 공유 계약). 이게 끝나야 두 트랙이 동시에 착수 가능.
   ...
   - 기존 `HANDOFF.md`(스프린트 1 스펙 아님, 내가 만들었던 "체력&데미지" 단독 스펙)는 위 방향 확정으로 사실상 대체됨 — 0단계 착수 시 `HANDOFF_sprint2_draft.md` 내용으로 교체하고 문서 구조 정리(CLAUDE.md 규칙대로) 진행 예정.
...
- **2026-09-08** — **스프린트 2 초안 접수 → 검토 → 팀 분업 확정.** 팀원이 `HANDOFF_sprint2_draft.md`("성장곡선 검증" — 체력/피격무적·처치보상·레벨EXP·골드강화5종·가상지역레벨·검증방법)를 `origin/main`에 push.
  ...
  - **트랙 분업 확정**: 트랙 A(팀원, Enemies/Systems 도메인) = 처치 보상 + 난이도 스케일링. 트랙 B(나, Core/Characters 도메인) = 레벨업 + 골드 강화 + 스탯 적용. 근거: 팀원은 최근(`d27af46`) `Enemy_Oni.prefab`/물리 세팅을 직접 다뤘고, 나는 `PlayerHealth`/`MageAttack`/`CharacterMover2D`를 이번 세션에서 직접 만들어 가장 잘 앎.
  - **공유 계약**: `Core.PlayerProfile`(`level, exp, gold, spUsed, upgrades{atk,hp,ms,atkSpeed,crit}`, `atk s`는 C# 예약어라 `atkSpeed`로 대체) + `Core.ProfileService.Current` 정적 접근자. 트랙 A는 `AddGold()`/`AddExp()` 메서드만 호출(필드 직접 증가 금지 — 레벨업 판정은 트랙 B 소유). `EnemyHealth`에 스탯 배율 setter(`SetMaxHp` 등) 필요 — `Awake()`가 이미 돈 뒤 `maxHp` 필드만 바꾸면 `CurrentHp`엔 반영 안 되는 이 프로젝트 단골 함정 주의(RequireComponent 자동보충·AddComponent Awake타이밍과 같은 종류).
  - 선행 작업(0단계: `feature/health-damage` 병합 + `PlayerProfile` 스캐폴딩)은 아직 미착수 — 다음 세션이 이어서 할 일.
- **2026-09-03(작성 시 08-29로 잘못 표기)** — **원본 전수 분석 → 포팅 로드맵 작성 + 스프린트 2(체력&데미지) 구현.** 사용자가 "프로토타입 전체적으로 분석하고 어떻게 업데이트할지 문서로 작성"을 요청.
  ...
  - 사용자가 이 순서를 승인하고 1번부터 착수 지시 → **스프린트 2(체력 & 데미지) 확정 후 실제로 구현.** `Core/IDamageable`·`Combat/DamageCalculator`·`Enemies/EnemyHealth`·`Characters/PlayerHealth` 신설, 공격 스크립트를 `Destroy()`→`TakeDamage()`로 전환, 접촉 데미지 활성화. 원본을 다시 읽어 넉백 비대칭(적=별도 성분+감쇠, 플레이어=처리 안 함이 원본과 같음) 확인, 스폰 보호 중 "움직이지도 때리지도 맞지도 않는다"(`:4022`)를 그대로 반영. PlayMode 테스트 30/30 통과(신규 10 + 기존 4건 갱신). **`feature/health-damage` 브랜치에 커밋(`c4518ad`) — 아직 `main` 미병합.**
  - CLAUDE.md "문서 구조" 규칙대로 기존 HANDOFF.md를 `docs/sprints/01-combat-core.md`로 `git mv`, HANDOFF.md는 이 스펙으로 교체 — 그러나 같은 날 팀원 초안이 도착해 스프린트 2 범위 자체가 위 항목(9월8일 로그)처럼 재확정됨.
