...
2. **다음 스프린트(신규 기능) 범위는 아직 미확정 — 팀 논의/승인 대기 중(2026-09-08 갱신).** 사용자가 성장곡선 검증(체력/보상/레벨/골드강화/가상 지역레벨) 초안을 `HANDOFF_sprint2_draft.md`로 업로드해 레포에 저장해둠(아래 로그 참고) — **아직 초안일 뿐 팀 승인 전이라 코드 구현 시작 안 함, 기존 `HANDOFF.md`도 미변경.** 세션이 임의로 정하지 말 것(HANDOFF.md 맨 아래 "따로 논의" 참고).
...
- **2026-09-08** — **스프린트 2 초안(`HANDOFF_sprint2_draft.md`) 접수 — 팀 검토 대기, 아직 미승인.** 사용자가 체력(HP)/피격 무적시간, 처치 보상(골드+EXP), 캐릭터 레벨/EXP, 골드 강화 5종, 난이도 스케일링 대리 지표("가상 지역 레벨"), 검증 방법(수치는 PlayMode, "체감 벽"은 직접 플레이)을 다루는 초안 문서를 업로드하며 반영을 요청.
  - PROGRESS.md 규칙("다음 스프린트 범위는 세션이 임의로 정하지 말 것")에 따라 어떻게 처리할지 먼저 확인 → 사용자가 **"레포에 초안 파일로만 저장"**을 명시적으로 선택.
  - 레포 루트에 `HANDOFF_sprint2_draft.md`로 그대로 커밋만 함 — **코드 구현(IDamageable, PlayerProfile, 골드 강화 등)은 전혀 시작하지 않았고, 기존 `HANDOFF.md`(1단계, 여전히 유효)도 변경 없음.**
  - 초안 내 "문서 구조" 절 명시: 팀 승인 나면 `HANDOFF.md`(1단계)를 `docs/sprints/01-combat-core.md`로 이동하고 이 초안이 새 `HANDOFF.md`(2단계)로 승격되는 절차가 남아있음 — 지금은 진행 안 함.
  - 초안 내 "확인 필요" 3건(플레이어 HP 0 처리 방식, 골드 강화 디버그 입력 방식, `regionLv` 증가 트리거 방식)은 팀 논의 항목으로 그대로 둠.
