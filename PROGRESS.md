...
**게임 한 바퀴가 전부 돈다** — 전투 코어 · 마법사 스킬트리 · 메카닉 0차 · 몬스터 7종 · 성소 · 보스 ·
런 사이클 · 로비 7탭 · 세이브 · 전투 배수 체인 · 경험치 구슬 · 지역 진행 · **윤회 · 아이템/가챠 · 업적**
까지 `main`에 병합·검증 완료. 미병합 작업 없음. **PlayMode 187/187 통과.**

**성장 고리가 닫혔다**: 사냥 → 지역 100마리 토벌 → 보스 → 다음 지역 → (지수 입장료 벽) →
**윤회** → 윤회 포인트 → **뽑기** → 아이템(영구) → 더 깊은 지역.

**이제 원본과 같은 흐름으로 돈다: 로비 → 지역 선택 → 90초 사냥 → 결과 → 로비.**
`Assets/Scenes/CombatCore.unity`를 열고 Play하면 **로비**부터 시작한다.

- **로비 탭 4개** — 캐릭터 선택 / 스테이지 / 강화 / 전문화. 여기서 캐릭터를 고르고 골드로
  강화를 사고 SP로 마법사 빌드를 찍은 뒤, 지역을 골라 입장한다(입장료 차감).
- **사냥 중** — ←/→ 이동, **C** 점프(원본은 C+Space지만 **의도적 편차로 C만**), **Z** 공격,
  **X** 전문화 스킬(빌드를 1층 이상 찍어야 나간다). **ESC**로 일시정지 → 로비로 나갈 수 있다.
- 90초가 끝나거나 죽으면 결과 화면(처치수·골드·경험치) → 로비.
- **진행이 저장된다** — 에디터를 껐다 켜도 레벨·골드·강화·전문화가 그대로다.
  (로비 "전체 초기화"로 지울 수 있다. 파일 위치는 `SaveService.SavePath`.)
- **전투 배수가 붙는다** — 연속으로 때리면 콤보(피해 +1.2%/콤보), 잡을수록 살기(피해 +0.8%/처치),
  0.8초 안에 3마리 잡으면 연쇄 보너스 골드. **내 레벨이 몹보다 낮으면 레벨당 -5%**(하한 25%)라
  상위 지역은 강화만으로는 안 뚫린다. HUD에 현재 배수가 보인다.

⚠️ **Tab 캐릭터 전환 / 숫자키 강화 / 6·7 빌드 선택은 전부 삭제됐다** — 원본에 없던 임시방편이고,
이제 로비에 정식 자리가 생겼다. R키 부활도 같이 삭제(원본은 죽으면 런이 끝난다).

몹은 3.6초마다 웨이브 스폰되고, **지역별 해금표**(원본 `rollSpawnType`)에 따라 종류가 정해진다 —
1지역은 도깨비불·오니만, 지역이 오르면 돌진귀→사수귀→분열귀가 차례로 추가되고 2지역부터 대오니도 섞인다.
도깨비불은 날아다니고, 돌진귀는 멈춰서 예고한 뒤 질주하고, 사수귀는 거리를 재며 화염탄을 쏘고,
분열귀는 죽으면 새끼 2마리를 남긴다. 8% 확률로 **엘리트**(체력×4·피해×1.5·크기×1.35·보상×5)가 섞인다.
처치하면 골드·경험치가 들어오고 레벨이 오른다.
...
4. ✅ **[나] 몹 7종 행동 + 지역별 스폰표 + 엘리트 완료** (`fb5e094`, PlayMode 94/94).
   종류별 행동은 `Enemies/IEnemyMotion` 계약을 구현한 전용 컴포넌트, 지역 표는 `Systems/EnemySpawnTable`,
   엘리트는 `Enemies/EnemyElite` 표식 + 스포너가 수치를 곱한다. 씬에 7종 전부 등록됨.
   ⚠️ 새 몹을 만질 때 걸리기 쉬운 세 가지는 `HANDOFF.md` "4~6번 실제 결과"에 정리해뒀다
   (`RefreshTypeBehaviour` 호출 필수 / 몸집은 스케일로 / 돌진귀 전이 프레임 순서).
5. ✅ **[나] 런 사이클 + 로비 UI 완료** (`9d5dd80`·`808e323`, PlayMode 113/113).
   `Systems/RunController`(로비↔사냥↔일시정지↔결과), `Core/RunTransient`(필드 정리),
   `Assets/Scripts/UI/`(로비·HUD·결과, IMGUI). 임시방편 3종 삭제.
5-1. ✅ **[나] 전투 배수 체인 + 세이브/로드** (`6d96e51`·`ffabf52`).
   `Core/CombatModifiers`(콤보·살기·연쇄·레벨페널티·성소버프), 히트스톱, `Systems/SaveService`.
5-2. ✅ **[나] 경험치 구슬 · 성소 · 지역 진행도 + 보스** (`c195339`·`9b16902`·`3ac149e`).
   지역 해금 사슬(100마리 → 보스 해금 → 격파 → 다음 지역)이 이어지면서 임시 전체 개방을 삭제.
5-3. ✅ **[나] 프로토타입 그림 추출·적용** (`74247f2`). 원본엔 이미지 파일이 없어서
   Canvas 드로잉을 PNG로 구워냈다 — 절차는 `docs/prototype-sprites.md`.
5-4. ✅ **[나] 아래키 낙하 + 보스 전용 무대** (`a1d4422`).
5-5. ✅ **[나] 윤회 + 윤회 장벽** (`8fa3390`), **아이템 35종 + 가챠** (`cfa1bd5`), **업적 32종** (`030b784`).
6. **다음: 섬영 0차 → 드루이드 0차** — 단계 1에서 유일하게 남은 조각. (팀원)
   지금 구현된 캐릭터는 **마법사·메카닉 2종뿐**이고, 전환도 Tab 디버그 키다.
   `PlayerRig`가 `ICharacterKit` 컴포넌트를 자동 등록하므로 **키트를 붙이기만 하면 된다**
   (계약은 `docs/worksplit.md` 3절). 원격에 팀원 브랜치는 아직 안 올라와 있다(2026-09-10 확인).
   키트를 붙이기만 하면 `PlayerRig`가 자동 등록하고 **로비 캐릭터 선택 탭에도 자동으로 뜬다.**
   원격에 팀원 브랜치는 아직 안 올라와 있다(2026-09-10 확인).
7. **그 뒤: 윤회 · 뽑기 · 업적 탭** — 영구 재화(요괴의 혼)와 아이템/업적 시스템이 필요해서
   로비 1차 구현에서 뺐다. 탭 자리만 비워둔 상태라 시스템이 생기면 탭만 추가하면 된다.
