...
| 7 | **로비** — 캐릭터 선택 / 강화 / 윤회 / 뽑기 (아래 "로비" 절 참고) | `:6974` 등 | 로비에서 캐릭터를 고르고 강화하고 런에 들어간다 |

> **⚠️ 7번은 A·B(섬영·드루이드 0차)가 끝난 뒤에 한다** — 2026-09-10 사용자 지시로 순서가 한 번 더 바뀌었다.
> 로비의 핵심 화면이 캐릭터 선택인데 선택지가 둘뿐이면 화면을 두 번 만들게 된다.
> 즉 **단계 1을 원래 정의대로(모든 캐릭터 0차 + 모든 몬스터) 끝낸 다음 로비**다.
...
[팀원] A 섬영 0차 → B 드루이드 0차            ← 여기가 단계 1의 마지막 남은 조각
        ↓
       A·B가 main에 들어온 뒤에야 7 로비(캐릭터 선택·강화·윤회·뽑기)
        ↓
