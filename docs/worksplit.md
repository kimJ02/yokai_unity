...
| 9 | **윤회 · 뽑기 · 업적 탭** — 영구 재화(요괴의 혼)·아이템·업적이 먼저 필요하다 | `:701`, `:547` | 로비에 탭 3개 추가 |

> **⚠️ 순서가 2026-09-10에 두 번 바뀌었다.** 처음엔 "로비를 A·B 뒤로" 였는데, 같은 날 사용자가
> *"섬영·드루이드는 팀원한테 맡기고 게임 시스템부터 구현하자 — tab으로 직업 바꾸지 말고
> 프로토타입처럼 로비로 나가서 찍게"* 라고 다시 정해서 **7번을 먼저 끝냈다**(`808e323`).
> 캐릭터 선택 탭은 `PlayerRig`가 붙어 있는 키트를 자동으로 읽으므로, 섬영·드루이드가 나중에
> 들어와도 **로비를 다시 만들 필요가 없다** — 원래 걱정했던 "화면을 두 번 만든다"가 해소됐다.
...
원래 로비는 단계 3 직전이었는데 **앞으로 당겨졌다**(9절 "범위 밖"에서 빠진 이유).

**단, 2026-09-10 재지시로 "몹 다음"이 아니라 "모든 캐릭터 0차 다음"이 됐다.**
사용자 지적: *"지금 마법사랑 메카닉만 구현된 거 아닌가? 그마저도 tab으로 바꾸는 식인데,
이렇게 하지 말고 모든 캐릭터를 구현하고 그 다음에 로비로 나가서 캐릭터를 바꾸게 하자."*
→ 캐릭터 선택 화면을 2종만 놓고 만들었다가 4종으로 다시 만드는 걸 피한다.
**Tab 디버그 전환은 로비가 생길 때까지 그대로 둔다**(지금 지우면 전환 수단이 아예 없어진다).

당긴 이유가 타당한 게, 지금 게임에 로비가 없어서 임시 조작이 계속 쌓이고 있다 —
캐릭터 전환은 디버그 키, 강화는 숫자 키(원본에 없음, 삭제 예정)로 붙어 있다. 로비가 생기면
이것들이 **정식 UI로 대체되면서 같이 지워진다**.

착수 전 확인할 것(원본 위치는 `docs/original-parity.md` "원본 위치 색인"):
- 로비 화면 자체와 진입/이탈(ESC) — 런 사이클(`GameState`) 계약이 이미 `main`에 있다
- 캐릭터 선택 → `PlayerRig.Select()`로 연결(계약 이미 있음, 디버그 키 삭제)
- 골드 강화(`:6974`) / 윤회 / 뽑기 — 어느 것이 `PlayerProfile`에 저장돼야 하는지 먼저 정리
...
- **로비는 "모든 캐릭터 0차"가 끝난 뒤다**(2026-09-10 재지시. 09-09엔 "몹 6종 다음"이었다).
  캐릭터 선택 화면을 2종으로 만들었다가 4종으로 다시 만드는 걸 피하기 위해서다. 원래 단계 3 직전이었는데 앞으로 당겼다 —
  로비가 없어서 캐릭터 전환(디버그 키)·강화(숫자 키) 같은 임시 조작이 계속 쌓이는 상태였다.
...
[팀원] A 섬영 0차 → B 드루이드 0차            ← **단계 1의 마지막 남은 조각**
        ↓
단계 2(캐릭터 하나씩 5차) → 9 윤회·뽑기·업적 → 단계 3(밸런스)
```
...
- 런 사이클·HUD·세이브 — 단계 3 직전 (**로비는 빠졌다** — 4절 7번으로 당겨졌다)
- 성소·보스·지역 진행/입장료·경험치 구슬·전투 배수 체인·아이템
  (**윤회·뽑기는 빠졌다** — 로비의 일부로 4절 7번에 들어갔다)
