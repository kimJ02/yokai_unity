...
> ## ✅ **2026-09-10 완료 — `9d5dd80`. 이 문서는 이제 기록이다.**
> 아래 스펙 그대로 실행했다. **현재 스펙이 아니므로 여기 적힌 걸 "할 일"로 읽지 말 것** —
> 현재 스프린트는 루트 `HANDOFF.md`, 현재 분업은 `docs/worksplit.md`.
>
> 실행하면서 스펙과 달라진 것:
> - **6번 "최소 OnGUI"가 정식 로비 UI로 커졌다** — 같은 날 사용자가 로비를 먼저 하라고 지시해서
>   런 사이클과 로비를 연달아 했다(`808e323`). `Assets/Scripts/UI/` 참고.
> - **`Core/RunTransient`가 추가됐다**(스펙엔 없던 것). 필드 정리를 "타입 목록"으로 하면 새 투사체가
>   생길 때마다 고쳐야 하고 빠뜨려도 에러가 안 나서, 표식 컴포넌트 방식으로 바꿨다.
> - 7번의 `PlayerDebugController` 삭제는 "로비 UI 때"로 미뤄뒀는데, 로비를 바로 이어서 만들어서
>   같이 지웠다.
