# [보류] 런 사이클 (게임의 뼈대) — 착수 전 보류

> ## ⏸️ **폐기가 아니라 보류다. 스펙은 아래 그대로 유효하다.**
> 2026-09-09 회의에서 진행 방식이 **"모든 캐릭터 0차 + 모든 몬스터 먼저"**로 바뀌면서 순서가 밀렸다.
> **단계 3(밸런스) 직전에 이 문서를 그대로 꺼내 실행한다** — 다시 설계하지 말 것.
>
> 계약(`Core/GameState`·`RunState`·`CombatEvents`·`IRunResettable`)은 **이미 `main`에 구현·검증돼 있다.**
> 남은 건 `Systems/RunController` 구현과 아래 7·8번(임시방편 삭제·지역 교체)뿐이다.
>
